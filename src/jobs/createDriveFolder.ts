...
export async function createDriveFolder(job: Job<JobData>): Promise<void> {
  ...
  const credentialsPath = process.env.GOOGLE_SERVICE_ACCOUNT_JSON ?? './credentials/google-service-account.json';
  if (!templateFolderId || !parentFolderId || !fs.existsSync(credentialsPath)) {
